...
public class BricksWave : MonoBehaviour {
	...
	void Start() {
		...
		if (transform.Find ("brickWaveText") != null) {
			waveText = transform.Find ("brickWaveText").GetComponent<TextMeshPro> ();
	...
	public void ColorBrick() {
		if (wave <= 30) {
			brick.material.SetColor("_Color", new Color (1, (1 - ((float)wave / 30)), 0));
		} else if (wave <= 60) {
			brick.material.SetColor("_Color", new Color (1, 0, (float)(wave - 30) / 30));
		} else {
			float redColorValue = 1-(((float)wave - 60) / 30);
			if (redColorValue < 1) {
				brick.material.SetColor("_Color", new Color (redColorValue, 0, 1));			
			} else {
				brick.material.SetColor("_Color", new Color (1, 0, 1));	
			}
		}
	}
}
